    ...
    <main className="w-full min-h-screen py-20">
      ...
      <section className="max-w-screen-lg mx-auto grid grid-flow-row-dense grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 py-10 px-4">
        {new Array(CHARACTERS_BW_COUNT).fill(0).map((_, i) => (
          <a
            key={i}
            href={`https://desilog-assets.sivaramp.com/static/characters/mono/${
              i + 1
            }.jpg`}
            target="_blank"
            className="relative"
          >
            <AvatarImage
              ...
              url={`https://desilog-assets.sivaramp.com/static/characters/mono/${
                i + 1
              }.jpg`}
              index={i}
            />
      ...
      <section className="max-w-screen-2xl mx-auto grid grid-flow-row-dense grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 py-10 px-4">
        {new Array(CHARACTERS_BW_COUNT).fill(0).map((_, i) => (
          <a
            key={i}
            href={`https://desilog-assets.sivaramp.com/raw/characters/mono/${
              i + 1
            }.png`}
            target="_blank"
            className="relative"
          >
            <AvatarImage
              ...
              url={`https://desilog-assets.sivaramp.com/raw/characters/mono/${
                i + 1
              }.png`}
              index={i}
            />
